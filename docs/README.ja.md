...
> **ステータス:** v0.3.0 - 品質改善とリリース整備アップデート
...
| **セルフライトボックス** | 画像（Core）, 段落, ポスター画像付き動画 | クリックで自身をライトボックス表示 |
...
動画ブロックの場合は、先にブロック設定でポスター画像を設定してから、ReformBox パネルの「クリックでライトボックス表示」を ON にします。
